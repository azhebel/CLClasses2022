...
# Create an empty dictionary of vocabularies
vocab = {}

# Cycle through each review
for x in reviews:
    # Take a review string, remove punctuation from it, convert all characters to lowercase,
    # split the string into a list of words, and then convert this list into a set (to get rid of duplicate words)
    normalized_review = set(reviews[x].translate(str.maketrans("", "", string.punctuation)).lower().split())

    # Assign the length of the set to the corresponding key in the dictionary of vocabularies
    vocab[x] = len(normalized_review)

    # Print the author and size of vocabulary
    print(x, "uses", len(normalized_review), "words")

print("---")
print(max(vocab, key=vocab.get), "has the largest vocabulary")

# Alternative way to get the max vocabulary
#
# vals = list(vocab.values())
# keys = list(vocab.keys())
#
# print(keys[vals.index(max(vals))], "has the largest vocabulary")
